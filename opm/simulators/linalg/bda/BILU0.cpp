...
BILU0<block_size>::~BILU0()
{
    ...
    template <unsigned int block_size>
    bool BILU0<block_size>::init(BlockedMatrix<block_size> *mat, BlockedMatrix<block_size> *jacMat)
    {
	const unsigned int bs = block_size;
	...
	if (opencl_ilu_reorder == ILUReorder::NONE) {
	    ...
	    CSCColPointers = new int[Nb + 1];
	    rmat = std::make_shared<BlockedMatrix<block_size> >(mat->Nb, mat->nnzbs);
	    rJacMat = std::make_shared<BlockedMatrix<block_size> >(jacMat->Nb, jacMat->nnzbs);
	    LUmat = std::make_unique<BlockedMatrix<block_size> >(*rJacMat);
...
template <unsigned int block_size>
bool BILU0<block_size>::create_preconditioner(BlockedMatrix<block_size> *mat, BlockedMatrix<block_size> *jacMat)
    {
        const unsigned int bs = block_size;
        auto *m = mat;
	auto *jm = jacMat;
	    ...
	    jm = rJacMat.get();
            Timer t_reorder;
            reorderBlockedMatrixByPattern<block_size>(mat, toOrder.data(), fromOrder.data(), rmat.get());
	    reorderBlockedMatrixByPattern<block_size>(jacMat, toOrder.data(), fromOrder.data(), rJacMat.get());
...
template bool BILU0<n>::init(BlockedMatrix<n>*,BlockedMatrix<n>*);		  \
template bool BILU0<n>::create_preconditioner(BlockedMatrix<n>*); \
template bool BILU0<n>::create_preconditioner(BlockedMatrix<n>*, BlockedMatrix<n>*);	\
